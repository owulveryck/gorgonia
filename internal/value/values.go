...
// Equaler represents any type that can perform a equal value check
type Equaler interface {
	ValueEq(Value) bool
}

// Closer represents any type that can perform a close-value check
...
// Grader is any type that has a value and a grad. This is used for Solvers
...
func TypeOf(v Value) hm.Type {
	...
	case tensor.Tensor:
		dt, dim := tensorInfo(t)
		return factory.MakeTensorType(dim, dt)
	case Scalar:
		return t.Dtype()
	case Typer:
		return t.Type()
